...
;
; psexec64 \\<redacted> /s /c /f /i C:\Users\<redacted>\System\Users\Biatu\Dev\AutoIt\<redacted>\Utils\_.Sources\RunAs_dev.exe <redacted> <redacted> 0 "\\<redacted>\...\Utils\prInst.exe \\<redacted>\<redacted>"
;
...
; Common Resource Dir
Global $sResourceDir="<redacted>"
Func _MapUtilityDrive()
	_InitAuth()
	ConsoleWrite("Mapping Utility Drive R:...")
	$sDesc=""
	$iRet=DriveMapAdd("R:",$sResourceDir,0,"<redacted>\"&$g_aAuth[0][0],_CryptUnprotectData(_Base64Decode($g_aAuth[0][1]),$sDesc))
	If @Error<>0 Then
